...
- `auth-service` отвечает за регистрацию, логин, профиль пользователя и выпуск JWT;
...
1. Регистрируется или входит в аккаунт и получает JWT.
2. Создаёт заказ с адресами, координатами и весом отправления.
3. Видит только собственные заказы.
4. Может использовать автоматическое назначение доступного курьера на свой заказ.

### Курьер

1. Регистрируется с ролью `courier` и получает профиль курьера, привязанный к `user_id` из JWT.
2. Видит свободные заказы со статусом `created` и собственные ранее назначенные заказы.
3. Может вручную принять свободный заказ только на свой courier-профиль.
4. Меняет статус назначенного заказа только в рамках разрешённого графа переходов.

Frontend использует те же JWT-защищённые контракты, что и backend: роль и идентичность пользователя не передаются клиентом как доверенные данные, а извлекаются из токена на стороне сервисов.
...
- REST API для авторизации, профиля, заказов и курьеров;
- отдельные auth-service и order-service;
- role-aware сценарии для `client` и `courier`;
- PostgreSQL, SQL-миграции и транзакционные сценарии;
- JWT-аутентификация и проверка токена в обоих защищённых API;
...
- регистрация принимает только поддерживаемые роли `client` и `courier`, а дальнейшие права определяются ролью из подписанного JWT;
- профиль курьера привязан к `user_id` из JWT, поэтому запрос не может выбрать другого курьера только подменой `courier_id`;
- клиентская выдача заказов ограничена владельцем, а курьерская выдача — свободными заказами и заказами, назначенными текущему курьеру;
- автоматическое назначение доступно клиенту только для собственного заказа, ручное принятие — курьеру только на собственный courier-профиль;
